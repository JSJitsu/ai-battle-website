...
var Game = Backbone.Model.extend({
  ...
  initialize: function() {
    ...
    var initialHtml = '<div class="row centered"><div class="dropdown">' +
         '<button class="btn btn-default dropdown-toggle" type="button" id="dropdownMenu1" data-toggle="dropdown">' + 
          ...
          '</ul>' +
        '</div>' +
      '</div><table class="leaderboard"></table>';
  ...
  updateViewingParams: function(clickEvent) {
    ...
    this.$el.find('.dropdown-toggle').append('<span class="caret"></span>');
  ...
  render: function() {
    var $table = this.model.find('table.leaderboard');
  ...
  // '</div>' +
    // '<table class="table table-bordered table-responsive table-striped lifetime-table leaderboard-table">' +
      ...
      var tableHtml = '<tr class="lifetime-table-header">' +
        '<td>Rank</td>' +
        '<td>Name</td>' +
